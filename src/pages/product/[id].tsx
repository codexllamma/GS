...
export default function ProductDetailsPage({
  ...
  return (
    <>
      <Header />
      <main className="grid grid-cols-[10%_55%_35%] h-screen overflow-hidden bg-neutral-50">
        
        {/* LEFT COLUMN — Thumbnails */}
        <div className="flex flex-col items-center justify-center gap-4 border-r border-neutral-200 p-4">
          {images.map((img, i) => (
            <button
              key={img.url}
              onClick={() => setCurrentImage(i)}
              className={`relative w-20 h-24 overflow-hidden rounded-md border transition-all ${
                currentImage === i ? "border-black" : "border-transparent hover:border-gray-400"
              }`}
            >
              <Image
                src={img.url}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <Image
                ...
                width={1100}
                height={1300}
                className="object-contain max-h-[97vh]"
                priority
              />
            </motion.div>
          </AnimatePresence>
        ...
        <aside className="p-10 flex flex-col justify-center space-y-10 overflow-hidden">
          <div>
          ...
          <div className="pt-6 border-t border-neutral-200 flex justify-center">
            <button
              ...
              className={`w-[400px] h-[53px] px-3 py-1 border transition-colors duration-300 overflow-hidden relative ${
                !selectedSize
