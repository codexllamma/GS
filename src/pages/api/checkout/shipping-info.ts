...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Use environment variables with fallback to verified credentials
    ...
    // 2. Query Courier Serviceability
    ...
    // 3. Map Shiprocket couriers into Razorpay format
    const shippingMethods = couriers.map((courier: any) => ({
      ...
      serviceable: true,
      shipping_fee: Math.round(Number(courier.rate || 0) * 100),
      cod: courier.cod === 1,
      cod_fee: Math.round(Number(courier.cod_charges || 0) * 100),
    }));
    ...
    // TEMPORARY: Expose exact error message to terminal curl
      ...
      addresses: [
        {
          ...
          shipping_methods: [
            {
              id: "1",
              name: "Standard Delivery (Fallback Error)",
